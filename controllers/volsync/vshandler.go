...
const schedulingIntervalStringLengthMin int = 2
...
func (v *VSHandler) ReconcileRD(
	...
	op, err := ctrlutil.CreateOrUpdate(v.ctx, v.client, rd, func() error {
		if err := ctrl.SetControllerReference(v.owner, rd, v.client.Scheme()); err != nil {
			cRefErr := fmt.Errorf("error setting controller ref, %w", err)
			l.Error(cRefErr, "unable to set controller reference")
	...
	if err != nil {
		return nil, fmt.Errorf("unable to create or update ReplicationDestination, %w", err)
	}
...
func (v *VSHandler) ReconcileRS(
	...
	if err != nil {
		return false, fmt.Errorf("unable to create or update ReplicationSource, %w", err)
	}
...
func (v *VSHandler) listByOwner(list client.ObjectList) error {
	...
	if err := v.client.List(v.ctx, list, listOptions...); err != nil {
		listErr := fmt.Errorf("error listing resources,  %w", err)
		v.log.Error(listErr, "Failed to list by label", "matchLabels", matchLabels)
...
func (v *VSHandler) EnsurePVCfromRD(rdSpec ramendrv1alpha1.VolSyncReplicationDestinationSpec) error {
	...
	if err != nil {
		if !kerrors.IsNotFound(err) {
			notFoundErr := fmt.Errorf("failed to get ReplicationDestination, %w", err)
			l.Error(notFoundErr, "ReplicationDestination not found")
	...
	if latestImage == nil || latestImage.Name == "" || latestImage.Kind != VolumeSnapshotKind {
		noLatestImgErr := fmt.Errorf("unable to find LatestImage from ReplicationDestination %s", rdInst.GetName())
		l.Error(noLatestImgErr, "No latestImage")

		return noLatestImgErr
	}
	...
	if err := v.validateSnapshotAndAddFinalizer(*vsImageRef); err != nil {
		noSnapErr := fmt.Errorf("unable to find VolumeSnapshot %s, %w", vsImageRef.Name, err)
...
func (v *VSHandler) ensurePVCFromSnapshot(rdSpec ramendrv1alpha1.VolSyncReplicationDestinationSpec,
	...
	if err != nil {
		pvcCreateErr := fmt.Errorf("unable to create or update PVC from snapshot, %w", err)
		l.Error(pvcCreateErr, "PVC createOrUpdate err")
...
func (v *VSHandler) validateSnapshotAndAddFinalizer(volumeSnapshotRef corev1.TypedLocalObjectReference) error {
	...
	if err != nil {
		...
		return fmt.Errorf("unable to get VolumeSnapshot, %w", err)
	}
...
func (v *VSHandler) addFinalizerAndUpdate(obj client.Object, finalizer string) error {
	if v.addFinalizer(obj, finalizer) {
		if err := v.client.Update(v.ctx, obj); err != nil {
			updateErr := fmt.Errorf("failed to add finalizer, %w", err)
			v.log.Error(updateErr, "error with update")
...
func ConvertSchedulingIntervalToCronSpec(schedulingInterval string) (*string, error) {
	...
	if len(schedulingInterval) < schedulingIntervalStringLengthMin {
